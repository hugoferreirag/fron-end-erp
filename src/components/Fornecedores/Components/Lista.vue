...
import { mapActions } from "vuex";
  export default {
    ...
    data() {
      return {
        ...
        fields: [
          ...
          { key: 'cnpj', label: 'CNPJ', sortable: true,class: 'text-left', sortDirection: 'desc' },
          { key: 'email', label: 'Email', sortable: true,class: 'text-left', sortDirection: 'desc' },
          { key: 'tel', label: 'Tel', sortable: true,class: 'text-left', sortDirection: 'desc' },
    ...
    computed: {
      ...
      fornecedores(){
          return this.$store.state.Fornecedor.fornecedores
      },
      user(){
    ...
    mounted() {
      ...
      this.totalRows = this.fornecedores.length
    },
    methods: {
        ...
        infoUser(id){
            ...
            await this.$http.get('/fornecedor')
            .then(result=>{
      ...
      lock(id){
        this.$http.put(`fornecedor/lock/${id}`)
        .then(res=>{
          console.log(res)
          if(res.status == 200){
      ...
      unlock(id){
        this.$http.put(`fornecedor/unlock/${id}`)
        .then(res=>{
          console.log(res)
          if(res.status == 200){
