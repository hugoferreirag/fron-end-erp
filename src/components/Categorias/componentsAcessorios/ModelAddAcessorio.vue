      ...
      <h5 style="color:white;font-weight:bold">Novo Modelo</h5>
            ...
            <div class="row">
              <div style="text-align:center" class="col-md-12">
                <h6>Tipo</h6>
                <input v-model="newModel.type" type="text" />
                <h6>Modelo</h6>
                ...
                <select v-model="newModel.provider">
                  <option v-for="product in fornecedoresProduct" :key="product.id"  :value="{name:product.name,id:product.id}">{{product.name}}</option>
                </select>
                <h6>{{newModel.provider}}</h6>
              </div>
            </div>
...
export default {
    ...
    computed:{
        fornecedoresProduct() {
      return this.$store.state.Fornecedor.fornecedorProductAcs;
    },
    },
    created(){
      this.setFornecedoresProductAcs()
    },
    
    methods:{
         
          ...mapActions('Categoria',['setModeloAcs']),
          ...mapActions('Fornecedor',['setFornecedoresProductAcs']),
        ...
        async adicionar(){
            ...
            await this.$http.post('/acessorios/categoria', acessorio)
            .then(result=>{
